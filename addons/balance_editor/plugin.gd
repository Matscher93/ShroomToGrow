...
## editor: quitting Godot stops it. Starting it twice is safe, server.py sees the
## port taken by an editor serving this project and just opens the page.
...
func _stop() -> void:
	...
	# _pid would only cover runs this plugin launched.
