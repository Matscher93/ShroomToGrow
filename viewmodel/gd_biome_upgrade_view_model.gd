...
var desc_text: String:
	get:
		...
		# What the upgrade lands on, in one generated sentence: a tier by name and
		# level, several of them, or every node. Appended rather than authored, so
		# the ten-tiers-one-wording problem the symbiosis track has cannot start
		# here too, and so a retargeted effect cannot leave a description behind.
		var reach := ScopeLabel.reach_sentence(def.effects) if def else ""
