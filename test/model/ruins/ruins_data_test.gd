...
## the roster's levels, and how both survive a save.
...
func test_adding_a_mission_hands_back_a_usable_instance_id() -> void:
	...
	assert_str(String(entry["hero_id"])).is_equal("grub")
...
func test_a_hero_is_found_by_the_mission_it_carries() -> void:
	_data.add(&"dig", &"grub", 0.0, 1.0, [])
	assert_bool(_data.find_by_hero(&"grub").is_empty()).is_false()
	assert_bool(_data.find_by_hero(&"stalker").is_empty()).is_true()
...
func test_an_unrecruited_hero_reads_as_rank_zero() -> void:
	assert_int(_data.level(&"grub")).is_equal(0)

func test_setting_a_rank_emits_once_and_only_on_a_change() -> void:
	var emitted := [0]
	_data.heroes_changed.connect(func() -> void: emitted[0] += 1)
	_data.set_level(&"grub", 1)
...
func test_a_full_board_round_trips() -> void:
	...
	assert_int(restored.level(&"grub")).is_equal(2)
